...
$pdf->SetTitle('Discus order form');
$pdf->SetSubject('Discus');
$pdf->SetKeywords('Aeros, Order form, Discus');
...
<svg viewBox="0 0 705 490" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" x="0px" y="0px" id="dsc_svg" preserveAspectRatio="xMinYMin meet">
	...
	<g id="top">
		<path id="rear_line_tv" d="M686.27,228.84l-325.86,-148.44l-252.52,47.22c-1.42,0.84 -2.4,1.64 -3.06,2.39l5.86,0.81l15.22,0.16l208.95,-32.28l232.96,120.12l63.75,22.75l63.35,8.43c-2.68,-7.76 -5.63,-14.83 -8.65,-21.16Z" style="fill:' . getColor("rear_line") . ';"/>
		' . $svg_ml_tv . '
...
//Wing Version
$pdf->MultiCell($fcolw, 0, "Wing Version", 0, 'L', 0, 0, 20, '', true, 0, false, true, 40, 'T'); // left cell
$versionText='';
if (!$dsca){$versionText = 'C';} 
if (array_key_exists("shortpack",$sbmopt) && $sbmopt["shortpack"] == "shortpack"){if($versionText != ''){ $versionText .= ', ';} $versionText .= 'B (shortpack)';}
if (array_key_exists("mosq",$sbmopt) && $sbmopt["mosq"] == "mosq"){if($versionText != ''){ $versionText .= ', ';}$versionText .= 'M (mosquito)';}
$pdf->MultiCell($scolw, 0, $versionText, 0, 'L', 0, 1, 65 , '', true, 0, false, true, 40, 'T'); // right cell
...
$pdf->MultiCell($scolw, 0, getFullNameByMat($ts_mat), 0, 'L', 0, 1, 65 , '', true, 0, false, true, 40, 'T'); // right cell

//Trailing edge
$pdf->MultiCell($fcolw, 0, "Trailing Edge", 0, 'L', 0, 0, 20, '', true, 0, false, true, 40, 'T'); // left cell
$pdf->MultiCell($scolw, 0, getFullNameByMat($te_mat), 0, 'L', 0, 1, 65 , '', true, 0, false, true, 40, 'T'); // right cell
...
//Middle Line
if($dsca || $sbmopt["middle_line"] == "no_ml"){$ml_text = 'No Middle Line';}
else{$ml_text = getFullName("middle_line");}
$pdf->MultiCell($fcolw, 0, "Middle Line", 0, 'L', 0, 0, 20, '', true, 0, false, true, 40, 'T'); // left cell
$pdf->MultiCell($scolw, 0, $ml_text, 0, 'L', 0, 1, 65 , '', true, 0, false, true, 40, 'T'); // right cell
...
$pdf->MultiCell($scolw, 0, getFullNameByMat($battens_mat), 0, 'L', 0, 1, 65 , '', true, 0, false, true, 40, 'T'); // right cell

//Ribs
$pdf->MultiCell($fcolw, 0, "Ribs", 0, 'L', 0, 0, 20, '', true, 0, false, true, 40, 'T'); // left cell
$pdf->MultiCell($scolw, 0, getFullNameByMat($ribs_mat), 0, 'L', 0, 1, 65 , '', true, 0, false, true, 40, 'T'); // right cell

//Letters AEROS
$lettersAValue = "";
if(array_key_exists("letters_a",$sbmopt) && $sbmopt["letters_a"] != "no_letters"){
	$lettersAValue = getFullName("letters_a");
	if (array_key_exists("let_out_a",$sbmopt) && $sbmopt["let_out_a"] == "outl"){$lettersAValue .= ', Outlined';}
}
else{$lettersAValue = 'No';}
$pdf->MultiCell($fcolw, 0, "Letters 'AEROS'", 0, 'L', 0, 0, 20, '', true, 0, false, true, 40, 'T'); // left cell
$pdf->MultiCell($scolw, 0, $lettersAValue, 0, 'L', 0, 1, 65 , '', true, 0, false, true, 40, 'T'); // right cell

//Letters Discus
$lettersValue = "";
if(array_key_exists("letters",$sbmopt) && $sbmopt["letters"] != "no_letters"){
	$lettersValue=getFullName("letters");
	if (array_key_exists("let_out",$sbmopt) && $sbmopt["let_out"] == "outl"){$lettersValue .= ', Outlined';}
	if (array_key_exists("opt_letters_panel",$sbmopt) && $sbmopt["opt_letters_panel"] == "on_bs")
		{$lettersValue .= ', On Bottom Surface';} else {$lettersValue .= ', On Rear Line';}
}
else{$lettersValue = 'No';}
$pdf->MultiCell($fcolw, 0, "Letters 'Discus'", 0, 'L', 0, 0, 20, '', true, 0, false, true, 40, 'T'); // left cell
...
if ($sbmopt["opt_order"] == "wing") {
	...
	//Uprights
	$pdf->MultiCell(25, 0, "Uprights:", 0, 'L', 0, 0, '', '', true, 0, false, true, 40, 'T'); // left cell
	if ($dsca){$uprightsText='Silver (Finsterwalder)';}else{$uprightsText='Silver (Litestream)';}
	$pdf->MultiCell(50, 0, $uprightsText, 0, 'L', 0, 0, '', '', true, 0, false, true, 40, 'T'); // right cell
	
	//battens
	$pdf->MultiCell(25, 0, "Battens:", 0, 'L', 0, 0, docMarginLeft + $wrWidth / 2, '', true, 0, false, true, 40, 'T'); // left cell
	if ($dsca){$battensText='Aluminium';} 
	elseif ($sbmopt["battens_option"] =='bat_alu'){$battensText='Aluminium';}
	else{$battensText='Carbon d=8.0mm';}
	$pdf->MultiCell(50, 0, $battensText, 0, 'L', 0, 1, '' , '', true, 0, false, true, 40, 'T'); // right cell
	
	//Speedbar
	$pdf->MultiCell(25, 0, "Speedbar:", 0, 'L', 0, 0, '', '', true, 0, false, true, 40, 'T'); // left cell
	if ($dsca){$speedbarText='Alu round';}else{$speedbarText='Alu Profiled';}
	$pdf->MultiCell(50, 0, $speedbarText, 0, 'L', 0, 0, '', '', true, 0, false, true, 40, 'T'); // right cell
	
	//Wires
	$pdf->MultiCell(25, 0, "Wires:", 0, 'L', 0, 0, docMarginLeft + $wrWidth / 2, '', true, 0, false, true, 40, 'T'); // left cell
	if ($dsca){$wiresText='d=2.5mm PVC covered';}else{$wiresText='d=2.0mm';}
	$pdf->MultiCell(50, 0, $wiresText, 0, 'L', 0, 1, '' , '', true, 0, false, true, 40, 'T'); // right cell
	
	$pdf->ln($ln_mm);
	if ($sbmopt["spare_uprights"] ==''){$sbmopt["spare_uprights"] = '0';}
	if ($sbmopt["spare_tipwands"] ==''){$sbmopt["spare_tipwands"] = '0';}
	if (array_key_exists("spare_lel",$sbmopt)){$spareLEL = $sbmopt["spare_lel"];}else{$spareLEL = 'No';}
	if (array_key_exists("spare_ler",$sbmopt)){$spareLER = $sbmopt["spare_ler"];}else{$spareLER = 'No';}
	$pdf->Write(0, 'SPARE PARTS' . "\n", '', 0, 'L', true, 0, false, false, 0);
	$pdf->MultiCell($wrWidth / 4, 0, 'Uprights: ' . $sbmopt["spare_uprights"], 0, 'L', 0, 0, '', '', true, 0, false, true, 40, 'T');
	$pdf->MultiCell($wrWidth / 4, 0, 'Tip Wands: ' . $sbmopt["spare_tipwands"], 0, 'L', 0, 0, '', '', true, 0, false, true, 40, 'T');
	$pdf->MultiCell($wrWidth / 4, 0, 'LE#3 Left: ' . $spareLEL, 0, 'L', 0, 0, '', '', true, 0, false, true, 40, 'T');
	$pdf->MultiCell($wrWidth / 4, 0, 'LE#3 Left: ' . $spareLER, 0, 'L', 0, 1, '', '', true, 0, false, true, 40, 'T');
